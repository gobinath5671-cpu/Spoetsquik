...
import { safeJsonStringifyArray } from "@/lib/utils";

/* POST /api/seed — seeds admin + sample events. Idempotent. */
export async function POST() {
  try {
    const results = { admin: false, events: 0, skipped: [] as string[] };

    // 1. Seed admin (spec §4.2)
    ...
    if (!existingAdmin) {
      ...
      results.skipped.push("admin already exists");
    }

    // 2. Seed sample events (if none exist)
    ...
    if (eventCount === 0) {
      ...
      const sampleEvents = [
        {
          ...
          prizesCashPrizes: "₹1,00,000 overall | ₹25,000 per event",
          prizesMedals: true,
        ...
        },
        {
          ...
          prizesCashPrizes: "Winner: ₹50,000 | Runner-up: ₹25,000",
          prizesMedals: true,
          prizesChampionship: true,
        ...
        },
        {
          ...
          prizesCashPrizes: "Singles: ₹20,000 | Doubles: ₹15,000 per pair",
          prizesMedals: true,
        ...
        },
        {
          ...
          prizesCashPrizes: "Winner: ₹40,000 | Runner-up: ₹20,000 | Best Player: ₹5,000",
          prizesMedals: true,
          prizesChampionship: true,
        ...
        },
        {
          ...
          prizesCashPrizes: "₹5,000 per gold | ₹3,000 per silver | ₹1,500 per bronze",
          prizesMedals: true,
          prizesChampionship: true,
      ...
      results.skipped.push(`${eventCount} events already exist`);
    }

    // 3. Update statuses based on date (spec §9.1)
    const allEvents = await db.event.findMany();
    for (const ev of allEvents) {
      const eventDate = new Date(ev.eventDate);
      const now2 = new Date();
      let newStatus = ev.status;
      if (eventDate < new Date(now2.getTime() - 24 * 60 * 60 * 1000)) {
        newStatus = "completed";
      } else if (eventDate < now2) {
        newStatus = "ongoing";
      }
      if (newStatus !== ev.status) {
        await db.event.update({ where: { id: ev.id }, data: { status: newStatus } });
      }
    }
    ...
    return NextResponse.json(
      { success: false, message: "Seed failed", error: String(e) },
      { status: 500 }
    );
  }
}
